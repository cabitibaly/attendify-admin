...
import { ActivityIndicator, Image, ImageBackground, Pressable, RefreshControl, ScrollView, Text, TouchableOpacity, View } from 'react-native'
...
const DetailEmploye = () => {
    ...
    const { employe, isLoading, refetch: refetchEmployee } = useFetchEmploye(Number(id))
    ...
    const changerDeSite = async () => {
        ...
        try {
            const data = await authenticatedRequest({
                url: `${DEV_API_URL}/compte/changer-de-site/${id}/${site}`,
                method: 'PATCH',
            })
            ...
            if (data?.status === 200) {
                ...
                refetchEmployee();
                setTimeout(() => editBottomSheetRef.current?.close(), 200)
            }
        } catch (error) {
            console.log("une erreur est survenue:", error)
        } finally {setIsloadingRequest(false)}
                        ...
                        >
                            ...
                            >
                                <View className="w-full flex-col items-center justify-start gap-6">
                                    <Text className="text-gris-1 text-2xl font-medium">Changer de site</Text>
                                    <View className="w-full flex-col items-center justify-start gap-6">
                                        <View className='w-full flex-col items-start justify-center gap-2'>
                                            <Text className='text-xl text-gris-10 font-medium'>nouveau site</Text>
                                            <Dropdown site={site} setSite={setSite} inBottomSheet />
                                        </View>
                                        <TouchableOpacity 
                                             disabled={isLoadingRequest}
                                             onPress={changerDeSite}
                                             activeOpacity={0.8} 
                                        ...
                                        >
                                            {
                                                isLoadingRequest ?
                                                    <ActivityIndicator size="small" color="#EEEEF0" />
                                                    :
                                                    <Text className='text-xl text-gris-12 font-medium'>Confirmer</Text>
                                            }                        
                                        </TouchableOpacity>
                                    </View>
                                </View>
